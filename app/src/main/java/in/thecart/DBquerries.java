...
import in.thecart.ui.my_cart.MyCartFragment;
import in.thecart.ui.my_mall.MyMallFragment;
import in.thecart.ui.my_orders.MyOrdersFragment;
import in.thecart.ui.my_rewards.MyRewardsFragment;
import in.thecart.ui.my_wishlist.MyWishlistFragment;
                                ...
                                if ((long) queryDocumentSnapshot.get("view_type") == 0) {
                                    ...
                                    for (long x = 1; x <= 1; x++) {
                                        ...
                                        horizontalProductScrollModelList.add(new HorizontalProductScrollModel(
                                                Objects.requireNonNull(queryDocumentSnapshot.get("product_ID_" + x)).toString()
                                                , Objects.requireNonNull(queryDocumentSnapshot.get("product_image_" + x)).toString()
                                                , Objects.requireNonNull(queryDocumentSnapshot.get("product_title_" + x)).toString()
                                                , Objects.requireNonNull(queryDocumentSnapshot.get("product_subtitle_" + x)).toString()
                                                , Objects.requireNonNull(queryDocumentSnapshot.get("product_price_" + x)).toString()
                                        ));
        ...
        firebaseFirestore.collection("USERS").document(FirebaseAuth.getInstance().getUid())
                ...
                .addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
                    ...
                    public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                        if (task.isSuccessful()) {
                            if (task.getResult() != null && task.getResult().get("list_size") != null) {
                                for (long x = 0; x < (long) task.getResult().get("list_size"); x++) {
                                    ...
                                    if (DBquerries.wishlist.contains(ProductDetailsActivity.productID)) {
                                        ...
                                        if (ProductDetailsActivity.addToWishlistBtn != null) {
                                            ProductDetailsActivity.addToWishlistBtn.setSupportImageTintList(context.getResources().getColorStateList(R.color.random));
                                        }
                                    } else {
                                        if (ProductDetailsActivity.addToWishlistBtn != null) {
